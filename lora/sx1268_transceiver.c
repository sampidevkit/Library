...
void sx1268_interface_receive_callback(uint16_t type, uint8_t *buf, uint16_t len)
{
    switch(type)
    {
        ...
        case SX1268_IRQ_RX_DONE:
        {
            ...
            __db("sx1268: rssi is %0.1f.\n", rssi_pkt);
            __db("sx1268: snr is %0.2f.\n", snr_pkt);
            __db("sx1268: signal rssi is %0.1f.\n", signal_rssi_pkt);
        ...
        {
            __db("sx1268: irq preamble detected.\n");

            break;
        }
        case SX1268_IRQ_SYNC_WORD_VALID:
        {
            __db("sx1268: irq valid sync word detected.\n");

            break;
        }
        case SX1268_IRQ_HEADER_VALID:
        {
            __db("sx1268: irq valid header.\n");

            break;
        }
        case SX1268_IRQ_HEADER_ERR:
        {
            __db("sx1268: irq header error.\n");

            break;
        }
        case SX1268_IRQ_CRC_ERR:
        {
            __db("sx1268: irq crc error.\n");

            break;
        }
        case SX1268_IRQ_CAD_DONE:
        {
            __db("sx1268: irq cad done.\n");

            break;
        }
        case SX1268_IRQ_CAD_DETECTED:
        {
            __db("sx1268: irq cad detected.\n");
...
uint8_t sx1268_send_test(void)
{
    ...
    if(res!=0)
    {
        __db("sx1268: stop timer on preamble failed.\n");
        (void) sx1268_deinit();
    ...
    if(res!=0)
    {
        __db("sx1268: convert to register failed.\n");
        (void) sx1268_deinit();
    ...
    if(res!=0)
    {
        __db("sx1268: set lora sync word failed.\n");
        (void) sx1268_deinit();
    ...
    if(res!=0)
    {
        __db("sx1268: get tx modulation failed.\n");
        (void) sx1268_deinit();
    ...
    if(res!=0)
    {
        __db("sx1268: set tx modulation failed.\n");
        (void) sx1268_deinit();
    ...
    if(res!=0)
    {
        __db("sx1268: set rx gain failed.\n");
        (void) sx1268_deinit();
    ...
    if(res!=0)
    {
        __db("sx1268: get tx clamp config failed.\n");
        (void) sx1268_deinit();
    ...
    if(res!=0)
    {
        __db("sx1268: set tx clamp config failed.\n");
        (void) sx1268_deinit();
...
uint8_t sx1268_receive_test(uint32_t s)
{
    ...
    if(res!=0)
    {
        __db("sx1268: set lora packet params failed.\n");
        (void) sx1268_deinit();
    ...
    if(res!=0)
    {
        __db("sx1268: get iq polarity failed.\n");
        (void) sx1268_deinit();
    ...
    if(res!=0)
    {
        __db("sx1268: set iq polarity failed.\n");
        (void) sx1268_deinit();
    ...
    if(res!=0)
    {
        __db("sx1268: lora continuous receive failed.\n");
        (void) sx1268_deinit();
    ...
    {
        /* finish receive test */
        __db("sx1268: finish receive test.\n");
        (void) sx1268_deinit();
    }
